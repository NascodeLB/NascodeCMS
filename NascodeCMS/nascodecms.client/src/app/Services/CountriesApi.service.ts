...
export class CountriesApi {
  ...
  getCountries(filter: PaginationFilter): Observable<ApiResponse> {
    ...
    return this.http.get<{ value: ApiResponse }>(`${this.baseUrl}api/Countries/AllCountries`, { params })
      .pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          ...
          return throwError(() => new Error(`Error in getCountries: ${error.statusText}`));
        })
      );
  }

  getCountryById(Id: string, languageId: number): Observable<CountryDto> {
    return this.http.get<{ value: CountryDto }>(`${this.baseUrl}api/Countries/GetCountry/${Id}/${languageId}`).pipe(
      ...
      catchError(error => {
        ...
        this.handleHttpError(error);
        return throwError(() => new Error('Error in getCountryById: ' + error));
      })
    );
  }
  ...
  submitCountry(country: CountryDto): Observable<any> {
    const url = `${this.baseUrl}api/Countries/SubmitCountry`;
   
    return this.http.post<{ ID: number }>(url, country).pipe(
      catchError((error: HttpErrorResponse) => {
        console.error("Error in submitCountry", error); // Log detailed error
        this.handleHttpError(error);
        return throwError(() => new Error('Error in submitCountry: ' + error.message));
      })
    );
  }


  deleteCountry(Id: number, languageId: number): Observable<any> { // Changed bankId to number if it's always an integer
    return this.http.post(`${this.baseUrl}api/Countries/DeleteCountry/${Id}/${languageId}`, null).pipe(
      tap(response => {
      ...
      catchError(error => {
        ...
        return throwError(() => new Error('Error in deleteCountry: ' + error));
      })
    );
  }

  deleteCountries(Ids: number[],languageId: number): Observable<any> {
    return this.http.post(`${this.baseUrl}api/Countries/DeleteManyCountries/${languageId}`, Ids).pipe(
      tap(response => {
      ...
      catchError(error => {
        ...
        return throwError(() => new Error('Error in deleteCountries: ' + error));
      })
    );
  }

  UpdateCountryStatus(Id: number, status: number, Language: number): Observable<any> { 
    
    return this.http.post(`${this.baseUrl}api/Countries/UpdateCountryStatus/${Id}/${status}/${Language}`, null).pipe(
      map(response => response),
      catchError(error => {
        console.log(error);
        this.handleHttpError(error);
        return throwError(() => new Error('Error in UpdateCountryStatus: ' + error));
      })
    );
  }
}
